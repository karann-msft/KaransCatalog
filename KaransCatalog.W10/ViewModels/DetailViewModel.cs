...
namespace KaransCatalog.ViewModels
{
    ...
    {
		private static SymbolIcon FavoriteSymbol = new SymbolIcon(Symbol.OutlineStar);
        private static SymbolIcon UnFavoriteSymbol = new SymbolIcon(Symbol.SolidStar);
        ...
        #region FavoriteIcon
        private IconElement _favoriteIcon;
		...
		private void UpdateFavoriteSymbolIcon()
        {
            if (SelectedItem != null)
            {
				var isOnFavorites = Singleton<UserFavorites>.Instance.IsOnFavorites(SectionName, SelectedItem);
				if (isOnFavorites)
				{
					FavoriteIcon = UnFavoriteSymbol;
				}
				else
				{
					FavoriteIcon = FavoriteSymbol;
				}
			}
        }	        
